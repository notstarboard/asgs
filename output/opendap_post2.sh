...
#
declare -a FILES_Standard
fileList=${properties["post.opendap.files"]}
IFS=' ' read -r -a FILES_Standard <<< "$fileList"

declare -a FILES_Additional
additionalFileList=${properties["post.opendap.additionalFiles"]}
IFS=' ' read -r -a FILES_Additional <<< "$additionalFileList"

declare -a FILES
FILES=( "${FILES_Standard[@]}" "${FILES_Additional[@]}" ) # NOTE: duplicates are not removed or detected
#
